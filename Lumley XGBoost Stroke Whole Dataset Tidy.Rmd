...
Diagnosis_WD$Diagnosis=as.factor(mapvalues(Diagnosis_WD$Diagnosis, c(0, 1), c("Mimic", "Stroke")))
...
############Check class ratio##########
Stroke_count=sum(Diagnosis_WD$Diagnosis=="Stroke")
Stroke_prop=Stroke_count/nrow(Diagnosis_WD)*100
Mimic_count=sum(Diagnosis_WD$Diagnosis=="Mimic")
Mimic_prop=Mimic_count/nrow(Diagnosis_WD)*100
Total_cases=nrow(Diagnosis_WD)

sprintf("N=%d/%d (%.2f%%) of cases were Stroke, N=%d/%d (%.2f%%) were Stroke Mimic", Stroke_count, 
...
set.seed(73)
DX_splitWD=initial_split(Diagnosis_WD, strata=Diagnosis) # stratify keep class ratio within each split
DX_TrainWD=training(DX_splitWD)    
DX_TestWD=testing(DX_splitWD)   
...
#Training Set 
Stroke_count_Tr=sum(DX_TrainWD$Diagnosis=="Stroke")
Stroke_prop_Tr=Stroke_count_Tr/nrow(DX_TrainWD)*100
Mimic_count_Tr=sum(DX_TrainWD$Diagnosis=="Mimic")
Mimic_prop_Tr=Mimic_count_Tr/nrow(DX_TrainWD)*100
Total_cases_Tr=nrow(DX_TrainWD)

sprintf("For the Training set, N=%d/%d (%.2f%%) of cases were Stroke, N=%d/%d (%.2f%%) were Stroke Mimic", Stroke_count_Tr, 
        Total_cases_Tr, Stroke_prop_Tr, Mimic_count_Tr, Total_cases_Tr, Mimic_prop_Tr)

#Testing Set 
Stroke_count_Te=sum(DX_TestWD$Diagnosis=="Stroke")
Stroke_prop_Te=Stroke_count_Te/nrow(DX_TestWD)*100
Mimic_count_Te=sum(DX_TestWD$Diagnosis=="Mimic")
Mimic_prop_Te=Mimic_count_Te/nrow(DX_TestWD)*100
Total_cases_Te=nrow(DX_TestWD)

sprintf("For the Testing set, N=%d/%d (%.2f%%) of cases were Stroke, N=%d/%d (%.2f%%) were Stroke Mimic", Stroke_count_Te, 
...
DX_Trainlbl=data.frame(DX_TrainWD)
DX_Trainlbl$Label="Train"

DX_Testlbl=data.frame(DX_TestWD)
...
Kfold_WD_samples=vfold_cv(DX_TrainWD, v=10, strata=Diagnosis)   
...
```{r}
#view first folds:
lapply(Kfold_WD_samples, head)
#view all folds:
Kfold_WD_samples$folds
#View evaluation results:
Kfold_WD_samples

####see training v testing set within each fold####
Kfold_WD_1 = get_rsplit(Kfold_WD_samples, index = 1) 
split_tib1=tidy(Kfold_WD_1)

Kfold_WD_2 = get_rsplit(Kfold_WD_samples, index = 2) 
split_tib2=tidy(Kfold_WD_2)

Kfold_WD_3 = get_rsplit(Kfold_WD_samples, index = 3) 
split_tib3=tidy(Kfold_WD_3)

Kfold_WD_4 = get_rsplit(Kfold_WD_samples, index = 4) 
split_tib4=tidy(Kfold_WD_4)

Kfold_WD_5 = get_rsplit(Kfold_WD_samples, index = 5) 
split_tib5=tidy(Kfold_WD_5)

Kfold_WD_6 = get_rsplit(Kfold_WD_samples, index = 6) 
split_tib6=tidy(Kfold_WD_6)

Kfold_WD_7 = get_rsplit(Kfold_WD_samples, index = 7) 
split_tib7=tidy(Kfold_WD_7)

Kfold_WD_8 = get_rsplit(Kfold_WD_samples, index = 8) 
split_tib8=tidy(Kfold_WD_8)

Kfold_WD_9 = get_rsplit(Kfold_WD_samples, index = 9) 
split_tib9=tidy(Kfold_WD_9)

Kfold_WD_10 = get_rsplit(Kfold_WD_samples, index = 10) 
split_tib10=tidy(Kfold_WD_10)

#Identifies the assessment set/actual fold data rather than whole split

KF_WD_1=as.data.frame(complement(Kfold_WD_1))
KF_WD_1=mutate(KF_WD_1, Fold=1)
colnames(KF_WD_1)[1]="N"

KF_WD_2=as.data.frame(complement(Kfold_WD_2))
KF_WD_2=mutate(KF_WD_2, Fold=2)
colnames(KF_WD_2)[1]="N"

KF_WD_3=as.data.frame(complement(Kfold_WD_3))
KF_WD_3=mutate(KF_WD_3, Fold=3)
colnames(KF_WD_3)[1]="N"

KF_WD_4=as.data.frame(complement(Kfold_WD_4))
KF_WD_4=mutate(KF_WD_4, Fold=4)
colnames(KF_WD_4)[1]="N"

KF_WD_5=as.data.frame(complement(Kfold_WD_5))
KF_WD_5=mutate(KF_WD_5, Fold=5)
colnames(KF_WD_5)[1]="N"

KF_WD_6=as.data.frame(complement(Kfold_WD_6))
KF_WD_6=mutate(KF_WD_6, Fold=6)
colnames(KF_WD_6)[1]="N"

KF_WD_7=as.data.frame(complement(Kfold_WD_7))
KF_WD_7=mutate(KF_WD_7, Fold=7)
colnames(KF_WD_7)[1]="N"

KF_WD_8=as.data.frame(complement(Kfold_WD_8))
KF_WD_8=mutate(KF_WD_8, Fold=8)
colnames(KF_WD_8)[1]="N"

KF_WD_9=as.data.frame(complement(Kfold_WD_9))
KF_WD_9=mutate(KF_WD_9, Fold=9)
colnames(KF_WD_9)[1]="N"

KF_WD_10=as.data.frame(complement(Kfold_WD_10))
KF_WD_10=mutate(KF_WD_10, Fold=10)
colnames(KF_WD_10)[1]="N"


K_Folds=bind_rows(KF_WD_1, KF_WD_2, KF_WD_3, KF_WD_4, KF_WD_5, KF_WD_6, KF_WD_7, KF_WD_8, KF_WD_9, KF_WD_10)
...
split_propsWD=map_dbl(Kfold_WD_samples$splits,
        function(x){
...
Model_1_CWD_spec = boost_tree(
  trees = 2000,    #same as nrounds 
...
mtry_max= ncol(Diagnosis_WD)-1

set.seed(73)
xgb_grid_CWD = grid_latin_hypercube(
  stop_iter(),
  tree_depth(),
  min_n(),
  loss_reduction(),
...
WD_Metrics=metric_set(mn_log_loss, roc_auc, accuracy)

set.seed(73) # reproducible 
xgb_tg_CWD = tune_grid(
    xgb_wf_CWD,
    resamples = Kfold_WD_samples,
    grid = xgb_grid_CWD,
    metrics=WD_Metrics,
...
xgb_tg_CWD$.predictions

Best_WDauc_params=show_best(xgb_tg_CWD, metric = "roc_auc")  
Best_WDauc_params

Best_WDauc=show_best(xgb_tg_CWD, metric = "roc_auc") %>%
  select(mean, .config)
Best_WDauc

Best_WDacc_params=show_best(xgb_tg_CWD, metric = "accuracy")
Best_WDacc_params

Best_WDacc=show_best(xgb_tg_CWD, metric = "accuracy") %>%
  select(mean, .config)
Best_WDacc
...
xgb_tg_mets_CWD=xgb_tg_CWD %>%
  collect_metrics()
xgb_tg_mets_CWD

xgb_tg_preds_CWD=xgb_tg_CWD %>%
  collect_predictions()
xgb_tg_preds_CWD

XGB_TG_AUCS_CWD=xgb_tg_mets_CWD%>%
  filter(.metric=="roc_auc")%>%
  arrange(desc(mean))
XGB_TG_AUCS_CWD

autoplot(xgb_tg_CWD$loss_reduction, metric="roc_auc", by=loss_reduction)   #by loss reduction 

#loss reduction 
xgb_tg_LR_CWD=xgb_tg_CWD %>%
  collect_metrics() %>%
  filter(.metric == "roc_auc") %>%
...
LR_tune_plot_CWD = xgb_tg_CWD  %>%
  ...
  theme_minimal()
print(LR_tune_plot_CWD)

#Min_N
Min_N_tune_plot_CWD = xgb_tg_CWD  %>%
  collect_metrics() %>% #Collect metrics from tuning
  mutate(tree_depth = factor(tree_depth)) %>%
  ...
  scale_color_viridis_d(option = "plasma", begin = .9, end = 0) + 
  theme_minimal()
print(Min_N_tune_plot_CWD)

#Sample Size
SampleSz_tune_plot_CWD = xgb_tg_CWD  %>%
  collect_metrics() %>% #Collect metrics from tuning
  mutate(tree_depth = factor(tree_depth)) %>%
  ...
  scale_color_viridis_d(option = "plasma", begin = .9, end = 0) + 
  theme_minimal()
print(SampleSz_tune_plot_CWD)

#mtry
mtry_tune_plot_CWD = xgb_tg_CWD  %>%
  collect_metrics() %>% #Collect metrics from tuning
  mutate(tree_depth = factor(tree_depth)) %>%
  ...
  scale_color_viridis_d(option = "plasma", begin = .9, end = 0) + 
  theme_minimal()
print(mtry_tune_plot_CWD)

#learn rate
learn_rate_tune_plot_CWD = xgb_tg_CWD  %>%
  collect_metrics() %>% #Collect metrics from tuning
  mutate(tree_depth = factor(tree_depth)) %>%
  ...
  scale_color_viridis_d(option = "plasma", begin = .9, end = 0) + 
  theme_minimal()
print(learn_rate_tune_plot_CWD)
```

##### Select best parameters #####
```{r}

best_auc_CWD = select_best(xgb_tg_CWD,metric="roc_auc")
best_auc_CWD

best_acc_CWD = select_best(xgb_tg_CWD, metric="accuracy")
best_acc_CWD

```

##### Finalise worklow using optimum parameters #####
```{r}

final_xgb_CWD = finalize_workflow(
  xgb_wf_CWD,
  best_auc_CWD
)
final_xgb_CWD
...
###Fit to resamples - only useful for evaluation when not using nested kf #####
```{r}

set.seed(73)
Trained_XGBWD=fit_resamples(final_xgb_CWD, resamples=Kfold_WD_samples, control=control_grid(save_pred=TRUE))

################Training set metrics for resamples############

#rename all _CWD

Trained_XGB_MetsWD=Trained_XGBWD%>%
  collect_metrics()

###########Training set predictions for resamples############
Trained_XGB_Preds=Trained_XGBWD%>%
  collect_predictions()
...
set.seed(73)
last_boost_CWD = last_fit(
  final_xgb_CWD,
  DX_splitWD, 
  metrics=WD_Metrics, 
  control=control_last_fit(verbose=TRUE, allow_par=FALSE))
last_boost_CWD

last_boost_Metrics_CWD = last_boost_CWD%>%
  collect_metrics()
last_boost_Metrics_CWD

```

##### Extract fit from workflow to use model for predictions etc downstream #####
```{r}
XGB_fit_WD=extract_fit_parsnip(last_boost_CWD)
XGB_fit_WD

xgb_model_WD = extract_fit_engine(XGB_fit_WD)
...
XGB_WD_Eval=xgb_model_WD$evaluation_log

xgb_model_WD$best_iteration
xgb_model_WD$best_ntreelimit
xgb_model_WD$best_score #logloss at best iteration
xgb_model_WD$best_msg
...
Eval_mets_WD=melt(
  xgb_model_WD$evaluation_log,
  id.vars="iter",
  variable.name="Set",
  value.name="Error"
)

Eval_plot_WD=ggplot(Eval_mets_WD, aes(iter, Error, group=Set))+
  geom_line(aes(color=Set), linewidth=1.5)+
    geom_vline(aes(xintercept=xgb_model_WD$best_iteration), color="black")+
  scale_y_continuous(limits=c(.55,.7), breaks=c(.55, .6, .65, .7))+
    scale_x_continuous(limits=c(0,2000), breaks=c(0, 500, 1500, 2000))+
  theme_classic() +
  scale_color_manual(values="blue", labels="Testing")+
  theme(plot.title=element_blank(),
        axis.title.x.bottom = element_blank(),
        axis.title.y.left = element_blank(),
        legend.position.inside = c(.94, .95),
...
XGB_preds_CWD=last_boost_CWD%>%
  collect_predictions()

colnames(XGB_preds_CWD)[2]="Mimic_Prediction"
colnames(XGB_preds_CWD)[3]="Stroke_Prediction"
colnames(XGB_preds_CWD)[4]="Row"
colnames(XGB_preds_CWD)[5]="Predicted_Class"

#Training version

#Make diagnosis numeric again and map values
DX_Train_num=DX_TrainWD
...
Predict_fnctTr=as.data.frame(predict(xgb_model_WD, DX_Tr_Mx))

Predict_fnctTr$Row=row(Predict_fnctTr)
DX_CCTr_num=data.frame(DX_TrainWD, row(DX_TrainWD)[, 1])
...
CWD_Tr_Mimic_auc =round(pROC::auc(XGB_ROCobj_Tr_M),3)

########ROC Curves for testing set  N=1151 ###########

#get sens spec and thresholds 
XGB_ROC_M=XGB_preds_CWD%>%
  roc_curve(Diagnosis, Mimic_Prediction) 
XGB_ROC_M

#pROC object for ggroc
XGB_ROCobj_M = roc(XGB_preds_CWD$Diagnosis, XGB_preds_CWD$Mimic_Prediction, levels=c(control="Stroke", case="Mimic"), direction="<")
CWD_Mimic_auc = round(pROC::auc(XGB_ROCobj_M),3)
CWD_Stroke_auc = round(pROC::auc(XGB_preds_CWD$Diagnosis,XGB_preds_CWD$Stroke_Prediction, levels=c(control="Stroke", case="Mimic"), direction="<"),4)
...
ROC_Curve_M=XGB_ROCobj_M%>%
  ...
  xlab("1-Specificity")+
  ylab("Sensitivity")+
  labs(title=sprintf("Mimic Prediction ROC Curve", " ", "AUC= ", CWD_Mimic_auc))+
...
ROC_Curve_M=XGB_ROCobj_Tr_M%>%
  ...
  xlab("1-Specificity")+
  ylab("Sensitivity")+
  labs(title=sprintf("Mimic Prediction ROC Curve", " ", "AUC= ", CWD_Tr_Mimic_auc))+
  theme(plot.title=element_text(hjust=.5),
        legend.text=element_blank(),
        legend.title=element_blank(),
        legend.position = "none",
...
MergedTT_ROCS=ggroc(TR_Tes_ROC, aes="color", size=1, legacy.axes = TRUE)+
  ...
  ggtitle(paste0("Mimic Prediction ROC Curve:", " ", "Train AUC= ", CWD_Tr_Mimic_auc, ", ","Test AUC= ", CWD_Mimic_auc
                 )) +
...
ggsave(plot=MergedTT_ROCS, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/ROC Curves", filename="Testing and training ROCs.png", height=6, width=8)
```
...
#default 
def_thresh=factor(ifelse(XGB_preds_CWD$Mimic_Prediction>.5,"Mimic","Stroke"))

#True class variable
Truth_Class=factor(XGB_preds_CWD$Diagnosis)
...
Zero_FPR_XEX = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.88,"Stroke", "Mimic"))
Zero_FPR_XEX

Min_FPR_XEX = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.85,"Stroke", "Mimic"))
Min_FPR_XEX

Min_FPR_EX = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.8,"Stroke", "Mimic"))
Min_FPR_EX

Min_FPR_TX = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.75,"Stroke", "Mimic"))
Min_FPR_TX

Min_FPR_TF = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.7,"Stroke", "Mimic"))
Min_FPR_TF

Min_FPR_maxx = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.68,"Stroke", "Mimic"))
Min_FPR_maxx

Min_FPR_max = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.65,"Stroke", "Mimic"))
Min_FPR_max

Min_FPR_mid = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.62,"Stroke", "Mimic"))
Min_FPR_mid

Min_FPR_min = factor(ifelse(XGB_preds_CWD$Mimic_Prediction<0.6,"Stroke", "Mimic"))
Min_FPR_min

Youden_Thresh = factor(ifelse(XGB_preds_CWD$Mimic_Prediction>Best_thresh[1,1], "Mimic", "Stroke"))
Youden_Thresh
...
```{r}
AUC=data.frame(last_boost_Metrics_CWD$.estimate[2])
...
XGB_DA_Stats_Youd=bind_cols(CWD_Mimic_auc, XGB_Stats_Youden)
...
XGB_Stats_def=pROC::coords(XGB_ROCobj_M, 0.5, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                         "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                         "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_def=bind_cols(CWD_Mimic_auc, XGB_Stats_def)
...
XGB_Stats_FPR_Zero=pROC::coords(XGB_ROCobj_M, 0.88, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                       "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                       "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_Zero=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_Zero)
...
XGB_Stats_FPR_XEX=pROC::coords(XGB_ROCobj_M, 0.85, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                         "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                         "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_XEX=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_XEX)
...
XGB_Stats_FPR_EX=pROC::coords(XGB_ROCobj_M, 0.8, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                          "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                          "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_EX=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_EX)
...
XGB_Stats_FPR_TX=pROC::coords(XGB_ROCobj_M, 0.75, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                       "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                       "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_TX=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_TX)
...
XGB_Stats_FPR_TF=pROC::coords(XGB_ROCobj_M, 0.7, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                         "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                         "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_TF=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_TF)
...
XGB_Stats_FPR_maxx=pROC::coords(XGB_ROCobj_M, 0.68, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                            "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                            "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_min_FPRX=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_maxx)
...
XGB_Stats_FPR_max=pROC::coords(XGB_ROCobj_M, 0.65, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                            "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                            "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_max=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_max)
...
XGB_Stats_FPR_mid=pROC::coords(XGB_ROCobj_M, 0.62, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                     "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                     "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_mid=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_mid)
...
XGB_Stats_FPR_min=pROC::coords(XGB_ROCobj_M, 0.6, ret=c("threshold", "tn", "tp", "fn", "fp", "sensitivity",
                                                     "specificity", "accuracy", "tpr", "tnr", "fnr", "fpr",
                                                     "npv", "ppv", "youden", "closest.topleft"))
XGB_DA_Stats_FPR_min=bind_cols(CWD_Mimic_auc, XGB_Stats_FPR_min)
...
path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/DA Statistics")
...
#.65 threshold
XGB_preds_CWD$Predicted_Class=ifelse(XGB_preds_CWD$Mimic_Prediction>.65, "Mimic", "Stroke")

True_Class=data.frame(XGB_preds_CWD$Diagnosis)
Pred_Class=data.frame(XGB_preds_CWD$Predicted_Class)
...
Pred_CCC_Plot=ggplot(XGB_preds_CWD, aes(Row, Mimic_Prediction, color=Diagnosis, fill=Predicted_Class))+
...
Pred_D_Plot=ggplot(XGB_preds_CWD, aes(x=Mimic_Prediction, y=Diagnosis, color=Diagnosis, fill=Predicted_Class))+
...
Pred_CCC_Plot=ggplot(XGB_preds_CWD, aes(Stroke_Prediction, Diagnosis, color=Diagnosis, fill=Predicted_Class))+
  geom_point(shape=21, size=1.5, stroke=1.5)+
...
Pred_TCCC_Plot=ggplot(XGB_preds_CWD, aes(x=Mimic_Prediction, y=Stroke_Prediction, color=Diagnosis, fill=Predicted_Class))+
...
ggsave(plot=Pred_TCCC_Plot, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Prediction Probability Plots", filename="Testing Predictions for 65 2.png", height=8, width=15)


#Stroke and Mimic probabilities with diagnoses# 
Pred_TCCC_Plot=ggplot(XGB_preds_CWD, aes(x=Mimic_Prediction, y=Stroke_Prediction, color=Diagnosis))+
...
ggsave(plot=Pred_TrCCC_Plot, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Prediction Probability Plots", filename="Training Predictions.png", height=8, width=15)

#Facet the plots for WD
Preds_facet=gridExtra::grid.arrange(Pred_TCCC_Plot, Pred_TrCCC_Plot, nrow=2)
Preds_facet

ggsave(plot=Preds_facet, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Prediction Probability Plots", filename=" Facet Predictions.png", height=10, width=16)
...
###Mini confusion matrices##### 
 ...
 #had to label the wrong way round to make it display correctly in the plot 
  ...
  Conf_Mat_FPR_Zero
    ...
    ggsave(plot=Conf_Mat_FPR_Zero, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .88.png", height=5, width=6)
  ...
  Conf_Mat_FPR_XEX
    ...
    ggsave(plot=Conf_Mat_FPR_XEX, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .85.png", height=5, width=6)
  ...
  Conf_Mat_FPR_EX
      ...
      ggsave(plot=Conf_Mat_FPR_EX, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .80.png", height=5, width=6)
  ...
  Conf_Mat_FPR_TX
      ...
      ggsave(plot=Conf_Mat_FPR_TX, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .75.png", height=5, width=6)
 ...
 Correct_7=ifelse(CM_FPR_DF_TF$Truth_Class_fct!=CM_FPR_DF_TF$Pred_Class, "Correct", "Incorrect")
  ...
  Conf_Mat_FPR_TF
      ...
      ggsave(plot=Conf_Mat_FPR_TF, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .70.png", height=5, width=6)
  ...
  ggsave(plot=Conf_Mat_FPR_max, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .65.png", height=5, width=6)
...
Correct_62=ifelse(CM_FPR_DF_mid$Truth_Class_fct!=CM_FPR_DF_mid$Pred_Class, "Correct", "Incorrect")
  ...
  Conf_Mat_FPR_mid
      ...
      ggsave(plot=Conf_Mat_FPR_mid, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .62.png", height=5, width=6)
  ...
  Conf_Mat_FPR_min
      ...
      ggsave(plot=Conf_Mat_FPR_min, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix .60.png", height=5, width=6)
  ...
  Conf_Mat_Youden
      ...
      ggsave(plot=Conf_Mat_Youden, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Confusion Matrix", filename="Confusion Matrix Youden.png", height=5, width=6)
...
```{r}
#Prepare dataframe
  Disc_WDdf=bind_cols(Preds_FPR_max, DX_TestWD)
  
  Disc_WDdf$Gender=mapvalues(Disc_WDdf$Gender, c(1, 2), c("Male", "Female"))
   Disc_WDdf$Irregular_Pulse=mapvalues(Disc_WDdf$Irregular_Pulse, c(0, 1), c("Regular", "Irregular"))
Disc_WDdf=Disc_WDdf%>%
  select_all(~gsub("_", " ",.))

 Disc_WDdf$"Misclassified Stroke"=ifelse(Disc_WDdf$"Min FPR max"=="Mimic" & Disc_WDdf$Diagnosis=="Stroke", "Incorrect", "Correct")
...
Test_N=bind_cols(DX_Ntst, Disc_WDdf)
...
SubtypeMG$Truth_Class=SubtypeMG$"Truth Class fct"
   Disc_WDdfstr=filter(SubtypeMG, Truth_Class=="Stroke")
   
   Disc_WDdfstr=Disc_WDdfstr%>%
  select(-"ICD Stroke Subtype", -N, -"Min FPR max", -"Truth Class", -Diagnosis)

   Discrep_tbl_WD=Disc_WDdfstr%>%
...
SubtypeMG$"Misclassified Mimic"=ifelse(Disc_WDdf$"Min FPR max"=="Stroke" & Disc_WDdf$Diagnosis=="Mimic", "Incorrect", "Correct")

Disc_WDdfmim=filter(SubtypeMG, Truth_Class=="Mimic")
Disc_WDdfmim=Disc_WDdfmim%>%
  select(-Diagnosis, -"Misclassified Stroke", -"ICD Stroke Subtype", -N, -"Min FPR max",-"Truth Class", -Diagnosis, -"ICD Mimic/Stroke/Subtype")


  Discrep_tblmim_WD=Disc_WDdfmim%>%
...
####Variable importance of all features########  
  ...
  ggsave(plot=VIP_col_all, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Variable Importance Plot", filename="Variable Importance Plot.png", height=8, width=11)
```

#####Make matrix to use for interactions and SHAP#####
```{r}
#make recipe to select predictors and make into matrix 
DX_rec=recipe(Diagnosis~., data=DX_TrainWD)
DX_prep=prep(DX_rec)
...
XG_IntsCC=EIX::interactions(xgb_model_WD, X_train)
...
ggsave(plot=FT_IntsCC, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Interactions", filename="Sum Gain Interactons.png", height=8, width=8)

```

#EIX Radar
```{r}
Radar_importance=EIX::importance(xgb_model_WD, X_train, option="both")

Radar_var_imp=EIX::importance(xgb_model_WD, X_train, option="variables")
Radar_var_imp

Radar_plot=plot(Radar_var_imp, radar=TRUE)+
...
ggsave(plot=Radar_plot, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Interactions", filename="VI Importance.png", height=8, width=8)

Radar_int_imp=EIX::importance(xgb_model_WD, X_train, option="interactions")
Radar_int_imp

Radarint_plot=plot(Radar_int_imp, radar=TRUE)+
  labs(title="Radar plot depicting various measures of feature importance 
...
ggsave(plot=Radarint_plot, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Interactions", filename="VI Interactons.png", height=8, width=8)
```
...
Shapvals=shap.prep(xgb_model_WD, 
                   X_train=X_train
) 

#Check mean scores 
Shap_values=shap.values(xgb_model_WD, X_train)
...
ggsave(plot=Shap_Plot, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Shap Summary Plot", filename="Shap Summary Plot.png", height=8, width=8)
...
ggsave(plot=Age_SPD, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Dependency Plots/Univariate", filename="Age DP.png")
...
ggsave(plot=Dep_grid, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Dependency Plots/Univariate", filename="Dependency Facet.png", height=15, width=9)
...
ShapvizObj=shapviz(xgb_model_WD, X_pred=data.matrix(ShapvizMx), X=ShapvizMx)

sv_importance(ShapvizObj) 

ShapvizintObj=shapviz(xgb_model_WD, X_pred=data.matrix(ShapvizMx), interactions=TRUE, X=ShapvizMx)
...
ggsave(plot=Shap_ints, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Interactions", filename="SHAP Interactons.png", height=14, width=17)
...
ShapintsCC=shap.prep.interaction(xgb_model_WD, X_train)
...
ggsave(plot=Strong_grid, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Dependence Plots", filename="Dependency Interactions Facet.png", height=12, width=22)
...
#shows vars and interactions for each of the 1000 trees 

lolly=lollipop(xgb_model_WD, X_train)
...
setwd("~/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Interactions")
...
##Correct stroke
Disc_WDdfstrcorr=filter(Disc_WDdfstr, `Misclassified Stroke`=="Correct")
Disc_WDdfstrcorrsub=Disc_WDdfstrcorr%>%
  select(-"Misclassified Stroke", -"ICD Mimic/Stroke/Subtype")
Disc_WDdfstrcorrsub=Disc_WDdfstrcorrsub%>%
  select_all(~gsub(" ", "_",.))

Stroke_Corr_Mx=Disc_WDdfstrcorrsub
X_predCS=data.matrix(Stroke_Corr_Mx)

shap_prepCS=shap.prep(xgb_model_WD, X_train=X_predCS)

shap_valuesCS=shap.values(xgb_model=xgb_model_WD, X_train=X_predCS)
...
ggsave(plot=CS_shapforce, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots/Group force plots", filename="CS force.png", height=8, width=11)

ShapvizObjSC=shapviz(xgb_model_WD, X_pred=X_pred, X=Stroke_Corr_Mx)

#Incorrect stroke

Disc_WDdfstrincorr=filter(Disc_WDdfstr, `Misclassified Stroke`=="Incorrect")
Disc_WDdfstrincorrsub=Disc_WDdfstrincorr%>%
  select(-"Misclassified Stroke", -"ICD Mimic/Stroke/Subtype")
Disc_WDdfstrincorrsub=Disc_WDdfstrincorrsub%>%
  select_all(~gsub(" ", "_",.))

Stroke_Incorr_Mx=Disc_WDdfstrincorrsub
X_predSI=data.matrix(Stroke_Incorr_Mx)

shap_prepIS=shap.prep(xgb_model_WD, X_train=X_predSI)

shap_valuesIS=shap.values(xgb_model=xgb_model_WD, X_train=X_predSI)
...
ggsave(plot=IS_shapforce, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots/Group force plots", filename="IS force.png", height=8, width=11)


#Correct mimic 
Disc_WDdfmimcorr=filter(Disc_WDdfmim, `Misclassified Mimic`=="Correct")
Disc_WDdfmimcorrsub=Disc_WDdfmimcorr%>%
  select(-"Misclassified Mimic")
Disc_WDdfmimcorrsub=Disc_WDdfmimcorrsub%>%
  select_all(~gsub(" ", "_",.))

Mimic_Corr_Mx=Disc_WDdfmimcorrsub
X_predCM=data.matrix(Mimic_Corr_Mx)

shap_prepCM=shap.prep(xgb_model_WD, X_train=X_predCM)

shap_valuesCM=shap.values(xgb_model=xgb_model_WD, X_train=X_predCM)
...
ggsave(plot=CM_shapforce, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots/Group force plots", filename="CM force.png", height=8, width=11)

#incorrect mimic
Disc_WDdfmimincorr=filter(Disc_WDdfmim, `Misclassified Mimic`=="Incorrect")
Disc_WDdfmimincorrsub=Disc_WDdfmimincorr%>%
  select(-"Misclassified Mimic")
Disc_WDdfmimincorrsub=Disc_WDdfmimincorrsub%>%
  select_all(~gsub(" ", "_",.))

Mimic_Incorr_Mx=Disc_WDdfmimincorrsub
X_predIM=data.matrix(Mimic_Incorr_Mx)

shap_prepIM=shap.prep(xgb_model_WD, X_train=X_predIM)

shap_valuesIM=shap.values(xgb_model=xgb_model_WD, X_train=X_predIM)
...
ggsave(plot=IM_shapforce, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots/Group force plots", filename="IM force.png", height=8, width=11)
...
DX_Testnodx=DX_TestWD%>%
  select(-Diagnosis)
ShapvizTEMx=data.matrix(DX_Testnodx)

ShapvizTEObj=shapviz(xgb_model_WD, X_pred=data.matrix(ShapvizTEMx), X=ShapvizTEMx)

#Randomly select correctly and incorrectly identified stroke and mimic patients 
XGB_preds_CWD$TRow=row(XGB_preds_CWD)[, 1]
set.seed(73)
rand_grp=XGB_preds_CWD%>%
...
CS_row=filter(XGB_preds_CWD, TRow==CS_row$`CS_case$TRow`)
CS_Trow=CS_row$TRow

IS_case=filter(rand_grp, Predicted_Class=="Mimic" & Diagnosis=="Stroke")
IS_row=as.data.frame(IS_case$TRow)
IS_row=filter(XGB_preds_CWD, TRow==IS_row$`IS_case$TRow`)
IS_Trow=IS_row$TRow

CM_case=filter(rand_grp, Predicted_Class=="Mimic" & Diagnosis=="Mimic")
CM_row=as.data.frame(CM_case$TRow)
CM_row=filter(XGB_preds_CWD, TRow==CM_row$`CM_case$TRow`)
CM_Trow=CM_row$TRow

IM_case=filter(rand_grp, Predicted_Class=="Stroke" & Diagnosis=="Mimic")
IM_row=as.data.frame(IM_case$TRow)
IM_row=filter(XGB_preds_CWD, TRow==IM_row$`IM_case$TRow`)
IM_Trow=IM_row$TRow

#Correct stroke
PtCSCCPred=XGB_preds_CWD$"Mimic_Prediction"[CS_Trow]
PtCSCCClass=XGB_preds_CWD$"Predicted_Class"[CS_Trow]
PtCSCCDx=XGB_preds_CWD$Diagnosis[CS_Trow]

#Incorrect stroke
PtISCCPred=XGB_preds_CWD$"Mimic_Prediction"[IS_Trow]
PtISCCClass=XGB_preds_CWD$"Predicted_Class"[IS_Trow]
PtISCCDx=XGB_preds_CWD$Diagnosis[IS_Trow]

#Correct mimic
PtCMCCPred=XGB_preds_CWD$"Mimic_Prediction"[CM_Trow]
PtCMCCClass=XGB_preds_CWD$"Predicted_Class"[CM_Trow]
PtCMCCDx=XGB_preds_CWD$"Diagnosis"[CM_Trow]

#Incorrect mimic
PtIMCCPred=XGB_preds_CWD$"Mimic_Prediction"[IM_Trow]
PtIMCCClass=XGB_preds_CWD$"Predicted_Class"[IM_Trow]
PtIMCCDx=XGB_preds_CWD$"Diagnosis"[IM_Trow]
...
ggsave(plot=shapvizwfCS, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots", filename="CS Individual Waterfall Plot 2025.png", height=9, width=13)
...
ggsave(plot=shapvizwfIS, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots", filename="IS Individual Waterfall Plot.png", height=9, width=13)
...
ggsave(plot=shapvizwfIM, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots", filename="IM Individual Waterfall Plot.png", height=10, width=13)
...
ggsave(plot=shapvizwfCM, path="H:/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Waterfall Plots", filename="CM Individual Waterfall Plot.png", height=10, width=13)
...
All_Trees=xgb.plot.tree(model = xgb_model_WD, show_node_id = TRUE, render=FALSE)
All_Trees$nodes_df$fillcolor=ifelse(All_Trees$nodes_df$shape=='rectangle', "#E4D8C6", "#B8EED2")
render_graph(All_Trees)

setwd("~/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Decision Trees")
...
Collapsed_Trees=xgb.plot.multi.trees(model = xgb_model_WD, trees = 0:4, show_node_id = TRUE, render=FALSE, features_keep = 5)
...
#Tree 1/1901
Tree_1=xgb.plot.tree(model = xgb_model_WD, trees = 0, show_node_id = TRUE, render=FALSE)
...
#First 5 trees
Five_trees=xgb.plot.tree(model = xgb_model_WD, trees = 0:4, show_node_id = TRUE, render=FALSE)
...
#Tree 1901/1901
Tree_1901=xgb.plot.tree(model = xgb_model_WD, trees = 1900, show_node_id = TRUE, render=FALSE)
Tree_1901$nodes_df$fillcolor=ifelse(Tree_1901$nodes_df$shape=='rectangle', "#E4D8C6", "#B8EED2")
render_graph(Tree_1901)

setwd("~/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Decision Trees")
export_graph(Tree_1901, "Tree_1901.png")
setwd("~/Documents")


#First 20 trees
Twenty_trees=xgb.plot.tree(model = xgb_model_WD, trees = 0:19, show_node_id = TRUE, render=FALSE)
...
#First 10 trees
Ten_trees=xgb.plot.tree(model = xgb_model_WD, trees = 0:9, show_node_id = TRUE, render=FALSE)
...
Stroke_Depth=xgb.plot.deepness(xgb_model_WD, which=c("max.depth"),pch=16, col=rgb(.5,0,.5,0.8), cex=1)
Stroke_Depth
...
ggsave(plot=Stroke_Depth, path="~/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis/Decision Trees", filename="Model Tree Depth.png", height=3, width=5)
...
Tree_text_D=xgb.dump(xgb_model_WD, with_stats = TRUE)
...
Tree_Info=as.data.frame(xgb.model.dt.tree(feature_names = NULL, model=xgb_model_WD, use_int_id = FALSE))
Tree_Info
Tree_Info=as.data.frame(xgb.model.dt.tree(feature_names = NULL, model=xgb_model_WD, text=Tree_text_D, use_int_id = FALSE))
...
xgb.save(xgb_model_WD, "XGB Stroke vs Mimic Model WD")

setwd("~/Documents/PhD/1. Retrospective Study (Machine Learning)/Analysis/XGB Results/Whole Dataset Analysis")
xgb_model_WD=xgb.load("XGB Stroke vs Mimic Model WD")
